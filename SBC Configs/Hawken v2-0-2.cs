...
namespace SBC {
	public class DynamicClass {
		...
		const int iReversePedalBoost	= 212;
		
		const int iGlowyGoodnessMin		= 0; //0-15 for glow effect
		...
		//Keyboard stuff - Because we have more buttons than a standard XBOX Controller
		...
		public void Initialize() {
			...
			controller.AddButtonKeyLightMapping(ButtonEnum.MainMonZoomIn,		true,baseLineIntensity,HOLOTAUNT,true);
			controller.AddButtonKeyLightMapping(ButtonEnum.MainMonZoomOut,		true,baseLineIntensity,NORMALTAUNT,true);
			controller.AddButtonKeyLightMapping(ButtonEnum.ToggleFilterControl,	true,baseLineIntensity,SHOWSCORES,true);
			controller.AddButtonKeyLightMapping(ButtonEnum.MultiMonOpenClose,	true,baseLineIntensity,TOGGLEMAP,true);
			controller.AddButtonKeyLightMapping(ButtonEnum.Start,				true,baseLineIntensity,ACTION,true);
		...
		public void mainLoop() {
			...
			if(!controller.GetButtonState(ButtonEnum.ToggleVTLocation)) {
				joystick.setAxis(1,32+(int)((double)controller.AimingX*0.75),HID_USAGES.HID_USAGE_RX);
				joystick.setAxis(1,32+(int)((double)controller.AimingY*0.75),HID_USAGES.HID_USAGE_RY);
			} else {
				joystick.setAxis(1,64+(controller.AimingX/2),HID_USAGES.HID_USAGE_RX);
				joystick.setAxis(1,64+(controller.AimingY/2),HID_USAGES.HID_USAGE_RY);
			}
			...
			bool bBoost	= false;
			if(System.Math.Abs(controller.RotationLever) > iRotationLeverDead) {
				bBoost=true;
				if(controller.RotationLever > iRotationLeverDead)
					joystick.setAxis(1,128,HID_USAGES.HID_USAGE_X);
				else
					joystick.setAxis(1,-128,HID_USAGES.HID_USAGE_X);
			}
			//Are we trying to jump?
			if(controller.LeftPedal > iLeftPedalDead)
				joystick.setButton(true,1,07);
			else
				joystick.setButton(false,1,07);
			
			//Should we boost?
			if(controller.RightPedal > iMovementPedalBoost ||
			  controller.MiddlePedal > iReversePedalBoost)
				bBoost=true;
			...
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.WeaponConSub,getGlowy(30));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.WeaponConMagazine,getGlowy(60));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.MainMonZoomOut,getGlowy(90));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.FunctionFSS,getGlowy(120));	//Dunno why, but the values are swapped for the LEDs...
			
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.Comm1,getGlowy(0));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.Comm2,getGlowy(18));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.Comm3,getGlowy(36));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.Comm4,getGlowy(54));
			controller.SetLEDState((ControllerLEDEnum)ButtonLights.Comm5,getGlowy(72));
		...
		public int getGlowy(int iOffset) {
			...
			double dSin = System.Math.Sin(angle);
			return (int)System.Math.Abs(dSin*(double)(iGlowyGoodnessMax-iGlowyGoodnessMin)) + iGlowyGoodnessMin;
		}
		...
		public void handleComms() {
			...
			if(controller.GetButtonState(ButtonEnum.Comm5) && !bComm5) {
				bComm5=true;
				Microsoft.DirectX.DirectInput.Key[] Message = {
					Microsoft.DirectX.DirectInput.Key.Y,
					Microsoft.DirectX.DirectInput.Key.T,
